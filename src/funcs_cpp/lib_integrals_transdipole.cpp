...
//      *************************************************************     //
//                      Transition Dipole Solution
//      Numerical Solution to the STOnG basis set for the transition
//      dipole integral values.
//      *************************************************************     //
/*                 ----Author: Justin Smith       ----
...
double TD_factor(int axyz,int bxyz,double an,double bm,double Axyz,double Bxyz)
{
        double rtnval = 0;
        ...
        for (int i = 0; i <= axyz ;++i)
        {
                for (int j = 0; j <= bxyz; ++j)
                {
                        //if ((i + j) == 0 || (i + j) % 2 == 0)
                        //{
                                rtnval += nCk(axyz,i) * nCk(bxyz,j) * pow(Pxyz - Axyz,(double)(axyz - i)) * pow(Pxyz - Bxyz,(double)(bxyz - j)) * TDNumerInt(i+j,an+bm,Pxyz);
                        //}
                }
        }
...
double calculate_TD_integral(int dim,int basis1,int basis2,MemHandler *data_mem,scf_data *scf,STOnG_handler *STO)
{
        double rtnval = 0;
	int N = STO->N;
			...
			{	
				for (int m = 0; m < N ;++m)
				{
					...
					double dm = STO[Bnum - 1].d[m + (N * basis_idx_col - N)];
        				double val = norm(ax,ay,az,an) * norm(bx,by,bz,bm) * dn * dm * TD_factor(ax,bx,an,bm,Ax,Bx) * S_factor(ay,by,an,bm,Ay,By) * S_factor(az,bz,an,bm,Az,Bz) * E_factor(an,bm,AmB) * (M_PI/(double)(an + bm));
				rtnval += val;
				}
			}
			break;
                ...
                case 1:
                {
                        for (int n = 0; n < N ;++n)
                        {
                                for (int m = 0; m < N ;++m)
                                {
                                        ...
                                        double dn = STO[Anum - 1].d[n + (N * basis_idx_row - N)];
                                        double dm = STO[Bnum - 1].d[m + (N * basis_idx_col - N)];
                                        double val = norm(ax,ay,az,an) * norm(bx,by,bz,bm) * dn * dm * S_factor(ax,bx,an,bm,Ax,Bx) * TD_factor(ay,by,an,bm,Ay,By) * S_factor(az,bz,an,bm,Az,Bz) * E_factor(an,bm,AmB) * pow(M_PI/(double)(an + bm),1);
                                rtnval += val;
                                }
                        }
			break;
                ...
                case 2:
                {
                        for (int n = 0; n < N ;++n)
                        {
                                for (int m = 0; m < N ;++m)
                                {
                                        ...
                                        double dn = STO[Anum - 1].d[n + (N * basis_idx_row - N)];
                                        double dm = STO[Bnum - 1].d[m + (N * basis_idx_col - N)];
                                        double val = norm(ax,ay,az,an) * norm(bx,by,bz,bm) * dn * dm * S_factor(ax,bx,an,bm,Ax,Bx) * S_factor(ay,by,an,bm,Ay,By) * TD_factor(az,bz,an,bm,Az,Bz) * E_factor(an,bm,AmB) * pow(M_PI/(double)(an + bm),1);
                                rtnval += val;
                                }
                        }
			break;
...
extern void TD_main(MemHandler *data_mem,scf_data *scf,dataOutput *optfile,STOnG_handler *STO)
{
	//Set some variables
	timer int_timer;
        optfile->ofile << "\nBeginning Transition Dipole Calculations...\n";
	int_timer.set_timer();
	...
	for (int row = 0; row < scf->orbitals; ++row)
	{
		for (int col = 0; col < scf->orbitals; ++col)
		{
			...
			double TDzij;
        		...
        		TDxij = calculate_TD_integral(0,row,col,data_mem,scf,STO);
        		TDyij = calculate_TD_integral(1,row,col,data_mem,scf,STO);
        		TDzij = calculate_TD_integral(2,row,col,data_mem,scf,STO);
			
			aif.rpl_2dval(row,col,TDxij,scf->TDx);
			aif.rpl_2dval(row,col,TDyij,scf->TDy);
			aif.rpl_2dval(row,col,TDzij,scf->TDz);
		}
	}
	...
	int_timer.end_timer();
	string message = "Transition Dipole Calculation Clock Time: ";
