	...
	{	
		for (int m = 0; m < N ;++m)
		{
			...
			//cout << "      an: " << an << " dn: " << dn << " bm: " << bm << " dm: " << dm << "\n";
        		double val = norm(ax,ay,az,an) * norm(bx,by,bz,bm) * dn * dm * S_factor(ax,bx,an,bm,Ax,Bx) * S_factor(ay,by,an,bm,Ay,By) * S_factor(az,bz,an,bm,Az,Bz) * E_factor(an,bm,AmB) * pow(M_PI/(double)(an + bm),1.5);
			rtnval += val;
