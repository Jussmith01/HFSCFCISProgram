...
double calc_sum_A(int ax,int ay,int az,int bx,int by,int bz,double Ax,double Ay,double Az, double Bx,double By,double Bz,double Cx,double Cy,double Cz,double an,double bm)
{
	...
	double neg = 0;
        ...
        double PmC = (Cx - Px) * (Cx - Px) + (Cy - Py) * (Cy - Py) + (Cz - Pz) * (Cz - Pz);
	...
	double Avalx = pow(-1,ix) * fi(ix,ax,bx,Px - Ax,Px - Bx) * A(ix,rx,ux,Px,Cx,ep);
	//cout << "I: " << ix - 2 * rx - ux << " iru: " << ix << rx << ux <<"\n";
        	...
        	double Avaly = pow(-1,iy) * fi(iy,ay,by,Py - Ay,Py - By) * A(iy,ry,uy,Py,Cy,ep);
                	...
                	double Avalz = pow(-1,iz) * fi(iz,az,bz,Pz - Az,Pz - Bz) * A(iz,rz,uz,Pz,Cz,ep);
			//cout << "         fact1: " << pow(-1,iz) << " fi: " << fi(iz,az,bz,Pz - Az,Pz - Bz) << " A: " << A(iz,rz,uz,Cz,Pz,ep) <<"\n";
			//cout << "         I: " << iz - 2 * rz - uz << " iru: " << iz << rz << uz <<"\n";
                
                                double nu = (ix + iy + iz) - 2.0 * (rx + ry + rz) - (ux + uy + uz);
				//cout << "         nu: " << nu << "\n";
                                double val = Avalx * Avaly * Avalz * Fnu(nu,PmC / (double)(4 * ep));
				//cout << "         Avalx: " << Avalx << " Avaly: " << Avaly << " Avalz: " << Avalz << " Fnu: " << Fnu(nu,PmC / (double)(4 * ep)) << "\n";
                                if (val > 0 && val > tol)
                                        {pos += val;}
...
extern void nuclear_main(int ac,MemHandler *data_mem,scf_data *scf,dataOutput *optfile,STOnG_handler *STO)
{
	...
	for (int row = 0; row < scf->orbitals; ++row) 
	{
		for (int col = 0; col < scf->orbitals; ++col)
		{
			...
			aif.add_2dval(row,col,Vij,scf->WM);
