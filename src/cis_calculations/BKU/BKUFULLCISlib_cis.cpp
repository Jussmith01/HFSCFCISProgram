...
void TEIMOTransform (scf_data *scf,vector<vector<vector<vector<double>>>> &vec4dMOSO)
{
	int nao = scf->orbitals;
	          ...
	          for(int q=0; q < nao; q++) {
	            for(int r=0; r < nao; r++) {
	              for(int s=0; s < nao; s++) {
	                ...
	                vec4dMOSO[i][j][k][l] += scf->C[i+p*nao] * scf->C[j+q*nao] * scf->C[k+r*nao] * scf->C[l+s*nao] * scf->FEI.get_val(p,q,r,s);
			cout << "  -p: " << p << " q: " << q << " r: " << r << " s: " << s << "\n";
	              }
	            }
	          }
	        }
		cout << "    VALUE: " << vec4dMOSO[i][j][k][l] << endl;	
	      }
	    }
	  }
	}
...
class TEIsb
{
	...
	vector<vector<vector<vector<double>>>> vec4dMOSO;
	...
	TEIsb (int N)
	{
		...
		for (int i=0;i<N;++i)
                ...
                for (int i=0;i<NM;++i)
                {
                        vec4dMOSO[i].resize(NM);
                }

                for (int i=0;i<NM;++i)
                        for (int j=0;j<NM;++j)
                        {
                                vec4dMOSO[i][j].resize(NM);
                        }
		...
		for(int p=0; p < N; p++)
        		...
        		for(int s=0; s < N; s++) 
			{
			  ...
			  //cout << "i: " << i << " j: " << j << " k: " << k << " l: " << l << endl;
        		  double value1 = vec4dMOSO[i][k][j][l] * (p%2 == r%2) * (q%2 == s%2);
			  if(value1 < 1.0E-14) {value1 == 0.0f;}
			  //cout << "TEST2\n";
        		  double value2 = vec4dMOSO[i][l][j][k] * (p%2 == s%2) * (q%2 == r%2);
			  if(value2 < 1.0E-14) {value2 == 0.0f;}
			  //cout << "TEST3\n";
        		  SaveElement(p,q,r,s,value1 - value2);
        ...
        void BuildHcoresb(scf_data *scf)
        {
                ...
                for(int p=0; p < N; p++)
                   for(int q=0; q < N; q++)
                   {
                          int i = p/2;
                          int j = q/2;
                          double value1 = scf->Hcore[j + i * scf->orbitals] * (p%2 == q%2);
			  if(value1 < 1.0E-14) {value1 == 0.0f;}
